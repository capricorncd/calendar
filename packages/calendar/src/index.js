...
ZxCalendar.prototype = {
  ...
  _initDom() {
    ...
    if (
      !options.hideFooter &&
      (options.mode === MODE_MULTIPLE || options.mode === MODE_RANGE)
    ) {
  ...
  createYears() {
    const [startRangeYear, endRangeYear] = getDateRange(
      this.options.dateRange,
      'yyyy'
    )
    const [startSelectedRangeYear, endSelectedRangeYear] =
      getSelectedDateRange(this)
    const years = []
   ...
   */
  createMonths() {
    const [startRangeMonth, endRangeMonth] = getDateRange(
      this.options.dateRange,
      'yyyyMM'
    )
    const [startSelectedRangeMonth, endSelectedRangeMonth] =
      getSelectedDateRange(this)
    const months = []
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      months.push({
        ...
        // range
        isRangeFirst:
          tempValue === startSelectedRangeMonth && endSelectedRangeMonth,
        isRangeLast:
          tempValue === endSelectedRangeMonth && startSelectedRangeMonth,
   ...
   */
  createDays() {
    const [startRangeDay, endRangeDay] = getDateRange(
      this.options.dateRange,
      'yyyyMMdd'
    )
    const [startSelectedRangeDay, endSelectedRangeDay] =
      getSelectedDateRange(this)
    ...
    const dates = days.map((day) => {
      ...
      tempItem = {
        ...
        // range
        isRangeFirst:
          tempValue === startSelectedRangeDay && endSelectedRangeDay,
